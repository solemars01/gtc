subroutine fftr1d(isign,irank,scale,x,y,icount)
  ...
  real(lk), intent(in) :: scale
  real(lk), intent(inout), dimension(0:irank-1) :: x
  complex(lk), intent(inout), dimension(0:irank/2) :: y
...
subroutine fftc1d(isign,irank,scale,x)
  ...
  real(lk), intent(in) :: scale
  complex(lk), intent(inout), dimension(0:irank-1) :: x
  complex(lk),  dimension(0:irank-1) :: y
...
! cout: complex array of output for Fourier modes=[0,n/2+1], normalized by n
  ...
  real(lk),intent(in) :: rin(n)
  real(lk),intent(in) :: scale
  complex(lk),intent(out) :: cout(n/2+1)  
  integer i
  complex(lk),dimension(n) :: fdata,dwork
...
! rout: real array for output, rout=\int{cin*exp(ikx)}dk
  ...
  real(lk),intent(in) :: scale
  complex(lk),intent(in) :: cin(n/2+1)
  real(lk),intent(out) :: rout(n)  
  integer i
  complex(lk),dimension(n) :: fdata,dwork
...
! cout: complex array of output for Fourier modes=[0,n-1], normalized by n
  ...
  implicit none
  integer,intent(in) :: isign,n
  complex(lk),intent(in),dimension(n) :: cin
  complex(lk),intent(out),dimension(n) :: cout
  complex(lk),dimension(n) :: dwork
  real(lk) :: scale
...
SUBROUTINE SPCPFT( A, B, C, UIN, UOUT, ISIGN )
  ...
  use precision,only:doubleprec,lk
